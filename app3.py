    ...
    for i in range(0, len(data), batch_size):
        ...
        for item in batch:
            ...
            if 'attachments' in item and item['attachments']:
             ...
             # ✅ 섹션 태깅: 본문/제목/첨부파일명에서 섹션 힌트 수집
            section_ids = set()
            section_ids.update(infer_section_ids(content))
            section_ids.update(infer_section_ids(item.get('title','')))
            if 'attachments' in item and item['attachments']:
                for attachment in item['attachments']:
                    section_ids.update(infer_section_ids(attachment.get('file_name','')))
            section_ids = list(section_ids)   

            # ✅ 같은 파일의 같은 섹션을 하나의 묶음으로 보는 key
            group_key = f"{source_file}:{','.join(sorted(section_ids))}"
...
def rebuild_bm25_and_hybrid():
    """FAISS 전체 문서로 BM25를 재생성하고, 하이브리드 리트리버를 갱신"""
    global bm25_retriever, hybrid_retriever, retriever, vectorstore
    try:
        vs = init_vectorstore()
        all_docs = _all_docs_from_faiss(vs)
        ...
        print("✅ BM25/하이브리드 리트리버 재구성 완료")
    except Exception as e:
        print(f"❌ BM25 재구성 오류: {e}")
...
def bundle_siblings(top_docs, all_docs, max_extra=5):
    """상위 문서와 같은 group_key(파일+섹션)인 청크를 이웃으로 더 붙여줌"""
    if not top_docs: return top_docs
    keys = { (_doc_feats(d)["group_key"]) for d in top_docs if _doc_feats(d)["group_key"] }
    if not keys: return top_docs
    extras = []
    for d in all_docs:
        if d in top_docs: continue
        if _doc_feats(d)["group_key"] in keys:
            extras.append(d)
            if len(extras) >= max_extra: break
    return top_docs + extras
...
# === Section Ontology (라이트) =========================
SECTION_PATTERNS = {
    "ITEM_LIST": ["급여 대상 품목","구입품목","대여품목","품목 목록","[급여 대상 품목]"],
    "COPAY":     ["본인부담률","부담률","공단부담금","본인 부담금","본인부담금"],
    "PROCEDURE": ["신청 절차","진행 절차","이용 절차","신청 방법","접수","제출 서류","필요 서류"],
    "ELIGIBILITY":["자격","대상","요건","기준","인정 기준","등급 기준"],
    "LIMITS":    ["한도액","연 한도액","사용 한도","월 한도"],
}
def infer_section_ids(text: str):
    if not text: return []
    q = text.lower().replace(" ","")
    found = set()
    for sec, keys in SECTION_PATTERNS.items():
        for k in keys:
            if k.lower().replace(" ","") in q:
                found.add(sec); break
...
def _doc_feats(d):
    ...
    return {
        ...
        "section_ids": set((m.get("section_ids") or [])),
...
키워드:"""
        ...
        try:
            ...
            docs = bundle_siblings(docs[:6], docs)
            filtered = filter_relevant_context(question, docs)
            return "\n\n".join([doc.page_content for doc in filtered])
...
def add_documents_to_vectorstore(new_documents):
    ...
    # ✅ BM25/하이브리드 리트리버 재구성 (증분 반영)
    rebuild_bm25_and_hybrid()
